...
class DB:
    ...
    def __call__(self, ligands_list):
        os.environ['OB_RANDOM_SEED'] = str(self.config['seed'])
       #return (DB.run_inference(ligands_list, **self.config))
        return self.run_inference(ligands_list, **self.config)
       #print(self.run_inference(ligands_list, **self.config))  
...
#       self.do(cmd)
        ligands = pd.DataFrame(ligands_list, columns=['ligand'])
        ligands['protein_path'] = proteinFile
