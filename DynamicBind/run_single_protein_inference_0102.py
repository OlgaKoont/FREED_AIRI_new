...
class DB:
    ...
    def __call__(self, smile):
       ...
       #return (DB.run_inference(smile, **self.config))
        return self.run_inference(smile, **self.config)
        print(self.run_inference(smile, **self.config))  
    ...
    def run_inference(self, smile, *, python, relax_python, results, proteinFile, num_workers,
...
#       self.do(cmd)
        ...
        if hts:
            ...
            self.do(cmd)
            cmd = f"MKL_SERVICE_FORCE_INTEL=1 MKL_THREADING_LAYER=GNU CUDA_VISIBLE_DEVICES={device_num} {python} {script_folder}/esm/scripts/extract.py esm2_t33_650M_UR50D data/prepared_for_esm_{header}.fasta data/esm2_output --repr_layers 33 --include per_tok --truncation_seq_length 10000 --model_dir {script_folder}/esm_models"
            self.do(cmd)
            cmd = f"MKL_SERVICE_FORCE_INTEL=1 MKL_THREADING_LAYER=GNU CUDA_VISIBLE_DEVICES={device_num} {python} {script_folder}/screening_old.py --seed {seed} --ckpt {ckpt} {protein_dynamic}"
            ...
            if not no_inference:
                ...
                cmd = f"{python} {script_folder}/datasets/esm_embedding_preparation.py --protein_ligand_csv {ligandFile_with_protein_path} --out_file data/prepared_for_esm_{header}.fasta"
                self.do(cmd)
                cmd = f"CUDA_VISIBLE_DEVICES={device_num} {python} {script_folder}/esm/scripts/extract.py esm2_t33_650M_UR50D data/prepared_for_esm_{header}.fasta data/esm2_output --repr_layers 33 --include per_tok --truncation_seq_length 10000 --model_dir {script_folder}/esm_models"
                self.do(cmd)
            ...
            if not no_relax:
                cmd = f"CUDA_VISIBLE_DEVICES={device_num} {relax_python} {script_folder}/relax_final.py --results_path {results}/{header} --samples_per_complex {samples_per_complex} --num_workers {num_workers}"
  ...
  #          for i in range(len(ligands)):
   #             cmd = f"CUDA_VISIBLE_DEVICES={device_num} {relax_python} {script_folder}/movie_generation.py {results}/{header}/index{i}_idx_{i} 1 --python {python} --relax_python {relax_python} --inference_steps {inference_steps}"
     #           self.do(cmd)
    #            print(cmd)