...
from rdkit.Chem import Lipinski, MolFromSmiles, QED
...
from ffreed.env.dynamicbind import DBAffinity
...
def init_rewards(args):
    ...
    costs = {
        'DockingScore': Reward(DockingVina(args['docking_config']), identity),
        ...
        'QED': Reward(QED.qed, neg, preprocess=preprocess),
        'DBAffinity': Reward(DBAffinity(args['db_config']), identity, batched_mode=True),
    }
    print("DBAffinity", DBAffinity(args['db_config']))
    rewards = dict()
    for name, weight in zip(args['objectives'], args['weights']):
        ...
        if name == 'DockingScore':
            rewards['DockingScore'] = CacheAndPool(costs[name], processes=processes)
        elif name == 'DBAffinity':
            rewards['DBAffinity'] = costs['DBAffinity']
        else:
