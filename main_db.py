...
from freedpp.DynamicBind.run_single_protein_inference_0102 import DB
...
from freedpp.args_db import parse_args, update_args
...
def init_rewards(args):
    ...
    costs = {
        'DB': Reward(DB(args['db_config']), OutOfRange(lower=9, upper=15, hard=hard)),
        ...
        'QED': Reward(qed, OutOfRange(lower=0.67, upper=1, hard=hard), preprocess=preprocess)
    }
    rewards = dict()
